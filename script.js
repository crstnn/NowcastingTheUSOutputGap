...
function getYearAndQuarter(val, spaceBetweenYearQuarter = false) {
    return String(~~val) + (spaceBetweenYearQuarter ? " " : "") + getQuarter(val);
}
...
async function graph(reqJSON) {

    document.getElementById("lastUpdate").innerHTML = "Last updated (UTC): " + reqJSON['latestRunUTC'].slice(0, reqJSON['latestRunUTC'].length - 3);
...
gapRequest.onload = function () {
    ...
    if (this.status === 200) {
        ...
        fig.innerHTML = '<br><br><br><br><b>Site momentarily undergoing maintenance. Please come back later.</b><br><br><br><br>';
    }
